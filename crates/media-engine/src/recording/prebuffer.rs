...
impl PreEventBuffer {
    pub async fn start(
        ...
        command
            ...
            .args([
                ...
                "mp4",
                "-segment_time",
                &BUFFER_SEGMENT_SECONDS.to_string(),
                "-reset_timestamps",
                "1",
                "-strftime",
                "1",
    ...
    pub async fn prune(&self) {
        let cutoff = Local::now() - chrono::Duration::seconds(i64::from(self.keep_seconds) + SAFETY_SEGMENTS);
        let Ok(mut entries) = fs::read_dir(&self.root).await else { return; };
        while let Ok(Some(entry)) = entries.next_entry().await {
            let path = entry.path();
            let Some(start) = buffered_segment_time(&path) else { continue; };
            if start < cutoff {
    ...
    pub async fn segments_before(
        ...
        let Ok(mut entries) = fs::read_dir(&self.root).await else { return result; };
        while let Ok(Some(entry)) = entries.next_entry().await {
            let path = entry.path();
            let Some(start) = buffered_segment_time(&path) else { continue; };
...
pub async fn supervise_pruning(buffer: std::sync::Arc<PreEventBuffer>, shutdown: CancellationToken) {
...
async fn cleanup_directory(root: &Path) {
    let Ok(mut entries) = fs::read_dir(root).await else { return; };
    while let Ok(Some(entry)) = entries.next_entry().await {
...
mod tests {
    ...
    fn parses_buffer_segment_timestamp() {
        ...
        assert_eq!(parsed.format("%Y-%m-%d %H:%M:%S").to_string(), "2026-08-20 10:16:44");
    }
}
